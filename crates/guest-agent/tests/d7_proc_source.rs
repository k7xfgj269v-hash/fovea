...
#[test]
fn proc_snapshot_contract_uses_u32_fd_count_and_carries_degradation_evidence() {
    let degradation = ProcDegradation::new(
        "identity.comm",
        "stat contains invalid UTF-8 and used U+FFFD replacement",
    );
    ...
    assert_eq!(snapshot.degradations, [degradation]);
}
